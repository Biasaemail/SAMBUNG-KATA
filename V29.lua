...
-- AUTO TYPE V29
-- ========================================================================
-- [FIX] Double huruf: typed mulai kosong, fireSim kirim suffix saja
-- [FIX] Book panel tidak overlap, Minimize ke TOP_H
-- [+]   Auto-Join: ChildAdded instant + fallback loop 1s
-- [+]   Cloud sync 1x di 5 detik pertama
-- [+]   Smart Endgame: trap akhiran sulit, stage-aware (aktif saat 2+ huruf)
-- [+]   Complete Index: prioritas kata yang belum pernah dipakai akun
-- [+]   Anti-repeat cross-match: variasi kata antar pertandingan
-- [+]   Semua mode scoring diimprove
-- [+]   Tab UI kompak: Config | Suffix | Suffix Chip Tags + Profile Manager
-- ========================================================================
...
local App = {
    ...
    State = {
        ...
        -- Cross-match: track akhiran & kata yang sering dipakai antar match (session)
        CrossMatchEndingCount = {},  -- [ending] = berapa kali dipakai session ini
        CrossMatchWordCount   = {},  -- [word]   = berapa kali dipakai session ini
        MatchCount            = 0,   -- jumlah match dalam session ini
        -- Complete Index: kata yang pernah dipakai akun (persisted ke file)
        IndexUsed = {},
    },
    DB = {
...
local function addWord(word, save)
    ...
    for i=1,math.min(4,#lw) do
        local p=sub(lw,1,i); if not App.DB.PrefixMap[p] then App.DB.PrefixMap[p]={} end
...
-- ========================================================================
-- [4] AUTO-JOIN — Deteksi instan (ChildAdded) + fallback loop 1s
-- ========================================================================
...
local function tryJoinTable(child)
    if not App.Config.AutoJoin or App.State.MatchActive then return end
    if not (child:IsA("Model") and child.Name:match("^Table_")) then return end
    ...
    if plrs > 0 and plrs < maxPlrs then
        if remotes:FindFirstChild("JoinTable") then
            remotes.JoinTable:FireServer(child.Name)
        end
    end
end
...
-- Deteksi instan: langsung coba join saat ada meja baru muncul
local tablesFolder = Workspace:FindFirstChild("Tables") or Workspace
tablesFolder.ChildAdded:Connect(function(child)
    task.wait(0.2)  -- beri waktu property InGame/Seat terinisialisasi
    tryJoinTable(child)
end)
-- Fallback loop 1s untuk detect perubahan occupant di meja yang sudah ada
task.spawn(function() while true do task.wait(1); scanAndJoinTable() end end)

-- ========================================================================
-- [5] SCORING ENGINE — Smart Endgame trap + Anti-repeat + Complete Index
-- ========================================================================
...
-- ── Akhiran sulit untuk bahasa Indonesia ─────────────────────────────────
-- Huruf tunggal yang jarang jadi awalan kata Indonesia:
local HARD_END_1 = { q=8, x=8, z=7, v=7, f=4, y=2 }
-- 2-huruf yang hampir tidak ada kata Indonesia berawal dengan itu:
-- (hanya aktif saat stage mid/late, ketika server letter sudah 2+ huruf)
local HARD_END_2 = {
    -- Benar-benar mustahil jadi awalan
    tt=10, rr=10, nn=10, ll=10, mm=10, pp=10, bb=10, dd=10, gg=10, kk=10,
    rt=9, lt=9, st=9, nt=9, kt=9, ft=9, pt=9,
    rl=9, lk=9, rk=9, sk=9, nk=8, mk=8,
    rs=9, ls=9, ns=8, ms=8, ts=8,
    lf=9, rf=9, nf=8, sf=8,
    rv=9, lv=9, nv=8,
    rz=9, lz=9, nz=9, tz=9, sz=9,
    rx=9, lx=9, nx=9, tx=9,
    rq=10, lq=10, nq=10, tq=10,
    -- Akhiran -ly, -cy, -gy, -ry (asing, susah dilanjut di indo)
    ly=8, cy=8, gy=7, ry=6, sy=7, ny=5, wy=9, vy=9, fy=9, zy=9,
    -- Akhiran vokal + konsonan langka
    ae=8, oe=7, ue=7,
    eh=7, ih=6, uh=6,
    -- Akhiran -it, -ot, -ut, -et (masih ada tapi tidak banyak)
    it=4, ot=4, ut=4,
    -- Kluster konsonan lain
    mb=3, nd=3, ng=2, nk=3,  -- ng masih oke, sisanya agak susah
}

-- Skor "kesulitan akhiran" sebuah kata (makin tinggi = makin susah dilanjut lawan)
local function trapScore(word)
    ...
    -- Skor dari 1-huruf terakhir (selalu dihitung)
    local base = (HARD_END_1[last1] or 0) * 15000

    -- Skor dari 2-huruf terakhir (hanya mid/late — saat server letter sudah 2 huruf)
    if s == "mid" or s == "late" then
        local hard2 = HARD_END_2[last2] or 0
        base = base + hard2 * 25000
    end

    -- Bonus tambahan: hitung berapa kata di DB yang berawal last1
    local oppOpts = App.DB.WordsStartingWith[last1] or 0
    if oppOpts == 0 then base = base + 500000 end   -- lawan pasti kalah!

    return base
end

-- ── Anti-repeat: record akhiran kata yang baru dipakai ───────────────────
local function recordEnding(word)
    if #word < 1 then return end
    local e2 = #word >= 2 and sub(word, -2) or sub(word, -1)
    -- per-match (8 slot terakhir)
...
-- Penalti repeat: makin sering dipakai (dalam match & antar match) makin besar penaltinya
local function repeatPenalty(word)
    if #word < 1 then return 0 end
    local e2 = #word >= 2 and sub(word, -2) or sub(word, -1)
    local pen = 0
    -- Penalti dalam match ini (akhiran sama)
    for _, v in ipairs(App.State.LastUsedEndings) do
        if v == e2 then pen = pen + 8000 end
    end
    -- Penalti cross-match (kata yang sama sering muncul antar pertandingan)
    local crossW = App.State.CrossMatchWordCount[word] or 0
    pen = pen + crossW * 12000
    -- Penalti cross-match akhiran
    local crossE = App.State.CrossMatchEndingCount[e2] or 0
    pen = pen + math.min(crossE * 3000, 60000)  -- cap agar tidak terlalu ekstrem
    return pen
end
...
local function scoreWord(word, mode)
    ...
    local oppOpts = App.DB.WordsStartingWith[last] or 0
    local wl      = #word
    local sb      = sfxBonus(word)
    local pen     = repeatPenalty(word)
    -- Noise: lebih besar saat early (variasi max), lebih kecil saat late (presisi)
    local s       = stage()
    local noiseMax = s == "early" and 8000 or s == "mid" and 3000 or 1000
    local ns      = random(1, noiseMax)

    if mode == "Smart Endgame" then
        -- ── Fokus utama: JEBAK lawan dengan akhiran sulit ──
        local trap = trapScore(word)
        -- Kalau sudah pasti menang (0 pilihan lawan)
        if oppOpts == 0 then return 9999999 - pen end
        -- Score = trap value + suffix bonus - repeat penalty + noise kecil
        -- Makin tinggi trap, makin diprioritaskan
        local base = trap + sb + ns - pen
        -- Sedikit bonus untuk kata panjang (harder for opponent to capitalize)
        base = base + (wl >= 7 and 15000 or wl >= 5 and 8000 or 0)
        return math.max(base, 1)

    elseif mode == "Menang Cepat" then
        -- ── Fokus: kata pendek + akhiran langka → lawan cepat kehabisan ──
        if oppOpts == 0 then return 9500000 - pen end
        local rareBonus = oppOpts == 0 and 500000
            or oppOpts <= 2  and 300000
            or oppOpts <= 5  and 150000
            or oppOpts <= 10 and 50000 or 0
        local shortBonus = math.max(0, (8 - wl)) * 9000
        -- Tambah sedikit trap score juga
        local trap = trapScore(word) * 0.4
        return math.max(shortBonus + rareBonus + trap + sb - pen + ns, 1)

    elseif mode == "Longest" then
        -- ── Fokus: kata sepanjang mungkin ──
        -- Bonus kecil kalau lawan masih punya banyak opsi (perpanjang permainan)
        local aliveBonus = oppOpts >= 10 and 5000 or oppOpts >= 5 and 2000 or 0
        return math.max(wl * 4000 + aliveBonus + sb - pen + ns, 1)

    elseif mode == "Shortest" then
        -- ── Fokus: kata sesingkat mungkin ──
        if oppOpts == 0 then return 9999999 - pen end
    ...
    elseif mode == "Complete Index" then
        -- ── Fokus: kata yang BELUM PERNAH dipakai akun sama sekali ──
        -- Kata baru (belum ada di IndexUsed) dapat bonus besar sekali
        local newBonus = App.State.IndexUsed[word] and 0 or 5000000
        -- Juga prioritaskan kata yang jarang dipakai (cross-match rendah)
    ...
    else -- Normal: pseudo-random tapi tetap anti-repeat
        local h = 0
        for i = 1, #word do h = (h * 31 + string.byte(word, i)) % 100000 end
        -- Session seed berubah tiap match → pola berbeda tiap game
...
local uiName = "AutoType_V29"
local pGui = (gethui and gethui()) or CoreGui
for _,n in ipairs({uiName,"AutoType_V28_Final","AutoType_V28_Tab","AutoType_V28_Ultimate","AutoType_V27_Ultimate"}) do
...
TitleL.BackgroundTransparency=1; TitleL.Text="AUTO TYPE V29"; TitleL.TextColor3=Color3.fromRGB(255,255,255)
...
local CloseB = mkTopBtn("✕",-4,  Color3.fromRGB(255,100,100),Color3.fromRGB(255,40,40))
local MinB   = mkTopBtn("—",-25, Color3.fromRGB(200,200,200),Color3.fromRGB(255,255,255))
local BookB  = mkTopBtn("📖",-47,Color3.fromRGB(150,200,255),Color3.fromRGB(255,255,255))
...
local ChipSF=Instance.new("ScrollingFrame"); ChipSF.Size=UDim2.new(1,0,0,21)
ChipSF.BackgroundColor3=Color3.fromRGB(11,13,23); ChipSF.BackgroundTransparency=0.28
...
local csK=Instance.new("UIStroke",ChipSF); csK.Color=Color3.fromRGB(85,165,255); csK.Transparency=0.58; csK.Thickness=0.7; ChipSF.Parent=P2
...
local function refreshChips()
    ...
    if #App.Config.TargetSuffixes==0 then
        local ph=Instance.new("TextLabel"); ph.Size=UDim2.new(0,130,1,0); ph.BackgroundTransparency=1
        ph.Text="Ketik akhiran di bawah"; ph.TextColor3=Color3.fromRGB(80,80,108); ph.Font=Enum.Font.Gotham; ph.TextSize=7; ph.Parent=ChipSF
        table.insert(chipObjs,ph); ChipSF.CanvasSize=UDim2.new(0,0,0,0); return
    end
    local tw=8
    for i,sfx in ipairs(App.Config.TargetSuffixes) do
        local col=TAG_C[((i-1)%#TAG_C)+1]; local cw=math.max(18,#sfx*7+10)
        local chip=Instance.new("Frame"); chip.Size=UDim2.new(0,cw,0,14); chip.BackgroundColor3=col; chip.BackgroundTransparency=0.48
        ...
        cl.Text=sfx:upper(); cl.TextColor3=Color3.fromRGB(255,255,255); cl.Font=Enum.Font.GothamBold; cl.TextSize=7; cl.Parent=chip
        table.insert(chipObjs,chip); tw=tw+cw+3
    end
    ChipSF.CanvasSize=UDim2.new(0,tw,0,0)
end

local SfxIn=Instance.new("TextBox"); SfxIn.Size=UDim2.new(1,0,0,20)
SfxIn.BackgroundColor3=Color3.fromRGB(13,15,27); SfxIn.BackgroundTransparency=0.18
SfxIn.Text=App.Config.TargetSuffixRaw; SfxIn.PlaceholderText="Cth: if,ax,cy,z,sme,vy"
SfxIn.TextColor3=Color3.fromRGB(255,255,255); SfxIn.PlaceholderColor3=Color3.fromRGB(75,75,105)
...
local sIs=Instance.new("UIStroke",SfxIn); sIs.Color=Color3.fromRGB(85,165,255); sIs.Transparency=0.48; sIs.Thickness=0.7; SfxIn.Parent=P2
...
PHdr.Text="📋  PROFIL TERSIMPAN"; PHdr.TextColor3=Color3.fromRGB(125,190,255)
PHdr.Font=Enum.Font.GothamBold; PHdr.TextSize=7; PHdr.TextXAlignment=Enum.TextXAlignment.Left; PHdr.Parent=P2

local PSF=Instance.new("ScrollingFrame"); PSF.Size=UDim2.new(1,0,0,54)
        ...
        for _,di in ipairs(App.DB.PrefixMap[sp] or {}) do
            ...
            if w and not seen[w] and not App.State.UsedWords[w]
               and not App.State.TriedThisTurn[w] and not App.State.PermanentBlacklist[w] then
                seen[w]=true
                if sub(w,1,#lp)==lp then table.insert(cands,{word=w,score=scoreWord(w,mode)}) end
            end
        end
    end
    ...
    remotes.PlayerCorrect.OnClientEvent:Connect(function(_,word)
        if word and type(word)=="string" then
            ...
            if lw and #lw>=2 then
                ...
                if not App.DB.KnownWords[lw] and not App.State.PermanentBlacklist[lw] then addWord(lw,true) end
            end
        end
...
if remotes:FindFirstChild("MatchUI") then
    remotes.MatchUI.OnClientEvent:Connect(function(cmd,value)
        if cmd=="ShowMatchUI" then
            ...
            -- Tiap match baru: seed baru + naikkan MatchCount untuk variasi cross-match
        ...
        elseif cmd=="UpdateServerLetter" then
            App.State.ServerLetter=value or ""; updateStatus()
        end
    end)
end

print("✅ Auto Type V29 Loaded!")
print("🧠 Smart Endgame: Trap akhiran sulit | Complete Index | Anti-repeat cross-match")
